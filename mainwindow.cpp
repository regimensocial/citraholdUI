...
MainWindow::MainWindow(QWidget *parent)
    ...
    citraholdServer =
        new CitraholdServer(configManager->getConfigProperty("serverAddress"),
    ...
    connect(ui->verifyButton, &QPushButton::clicked, this,
            &MainWindow::handleVerifyButtonClicked);

    connect(ui->tokenText, &QPlainTextEdit::textChanged, this,
            &MainWindow::manageTokenTextEdit);

    connect(ui->uploadButton, &QPushButton::clicked, this,
            &MainWindow::handleUploadButton);

    connect(ui->selectPreviousGameIDsButton, &QPushButton::clicked, this,
            &MainWindow::openGameIDSelector);

    connect(ui->extdataRadio, &QRadioButton::clicked, this,
            &MainWindow::handleSaveExtdataRadios);
    connect(ui->saveRadio, &QRadioButton::clicked, this,
            &MainWindow::handleSaveExtdataRadios);
    connect(ui->fetchButton, &QPushButton::clicked, this,
            &MainWindow::handleServerFetch);

    connect(ui->downloadButton, &QPushButton::clicked, this,
            &MainWindow::handleDownloadButton);

    connect(ui->clearOldSavesButton, &QPushButton::clicked, this,
            &MainWindow::handleClearOldSavesButton);

    connect(ui->switchModeButton, &QPushButton::clicked, this,
            &MainWindow::handleToggleModeButton);

    connect(ui->aboutCitraholdOption, &QAction::triggered, this,
            &MainWindow::openAboutWindow);
    connect(ui->openConfigFolderOption, &QAction::triggered, this,
            &MainWindow::openConfigFolder);
    connect(ui->aboutCitraholdOption, &QAction::triggered, this,
            &MainWindow::openAboutWindow);
    connect(ui->openSettingsOption, &QAction::triggered, this,
    ...
    QString userID =
        citraholdServer->verifyTokenToSetUserID(configManager->getToken());
    ...
    if (!citraholdServer->checkServerIsOnline()) {
        showErrorBox(
            "The server is inaccessible at this time. Please try again later.");
    } else {
        QStringList versions = citraholdServer->getSoftwareVersions();

        if (!versions.contains("v" + QCoreApplication::applicationVersion()) &&
            (configManager->getConfigProperty("ignoreVersion") !=
             versions[0])) {
            QMessageBox msgBox;
            QString message =
                "There is a new version of Citrahold PC available (" +
                versions[0] + "). Would you like to download it?";
            ...
            QAbstractButton *pButtonIgnore =
                msgBox.addButton(tr("Don't remind me"), QMessageBox::NoRole);

            msgBox.exec();

            if (msgBox.clickedButton() == pButtonYes) {
                QDesktopServices::openUrl(
                    QUrl("https://github.com/regimensocial/citraholdUI/"
                         "releases/latest"));
            } else if (msgBox.clickedButton() == pButtonIgnore) {
                configManager->updateConfigProperty("ignoreVersion",
                                                    versions[0]);
            }
        }
    }
...
void MainWindow::changeLastUploadText() {
    QDateTime lastUploadTime = configManager->getLastUploadTime(
        MainWindow::savesOrExtdata(), ui->uploadGameIDComboBox->currentText());

    if (lastUploadTime.toString("yyyy") == "1970" ||
        lastUploadTime.toString("yyyy") == "") {
        ui->lastDeviceUploadLabel->setText("");
    } else {
        ui->lastDeviceUploadLabel->setText(
            "Last upload on this device: " +
            lastUploadTime.toString("dd/MM/yyyy hh:mm:ss"));
        ui->lastDeviceUploadLabel->setToolTip(
            "day/month/year hour:minute:second");
    }
}

void MainWindow::changeLastDownloadText() {
    QDateTime lastDownloadTime = configManager->getLastDownloadTime(
        MainWindow::savesOrExtdata(),
        ui->downloadGameIDComboBox->currentText());

    if (lastDownloadTime.toString("yyyy") == "1970" ||
        lastDownloadTime.toString("yyyy") == "") {
        ui->lastDeviceDownloadLabel->setText("");
    } else {
        ui->lastDeviceDownloadLabel->setText(
            "Last download on this device: " +
            lastDownloadTime.toString("dd/MM/yyyy hh:mm:ss"));
        ui->lastDeviceDownloadLabel->setToolTip(
            "day/month/year hour:minute:second");
    }
}
...
void MainWindow::handleVerifyButtonClicked() {
    ...
    if (ui->tokenText->toPlainText().length() <=
        6) { // Handle a shorthand token

        QString fullToken = citraholdServer->getTokenFromShorthandToken(
            ui->tokenText->toPlainText());
...
void MainWindow::handleSuccessfulLogin() {
    ...
    if ((configManager->getConfigProperty("lastDownloadedGameID") != "") &&
        (ui->downloadGameIDComboBox->findText(
             configManager->getConfigProperty("lastDownloadedGameID")) != -1)) {
        ui->downloadGameIDComboBox->setCurrentText(
            configManager->getConfigProperty("lastDownloadedGameID"));
    }

    if ((configManager->getConfigProperty("lastUploadedGameID") != "") &&
        (ui->uploadGameIDComboBox->findText(
             configManager->getConfigProperty("lastUploadedGameID")) != -1)) {
        ui->uploadGameIDComboBox->setCurrentText(
            configManager->getConfigProperty("lastUploadedGameID"));
    }
}
...
UploadType MainWindow::savesOrExtdata() {
    return (ui->extdataRadio->isChecked() ? UploadType::EXTDATA
                                          : UploadType::SAVES);
}
...
void MainWindow::retrieveGameIDList(QString gameID) {
    ...
    if ((previousGameID != "") &&
        (ui->uploadGameIDComboBox->findText(previousGameID) != -1)) {
...
void MainWindow::handleUploadButton() {
    ...
    QString directory =
        configManager
            ->getGamePathFromGameID(MainWindow::savesOrExtdata(), gameID)
            .string()
            .c_str();
    ...
    QDateTime lastUploadTime =
        configManager->getLastUploadTime(MainWindow::savesOrExtdata(), gameID);
    QDateTime lastDownloadTime = configManager->getLastDownloadTime(
        MainWindow::savesOrExtdata(), gameID);
    QDateTime lastServerUploadTime = citraholdServer->getLastUploadTime(
        MainWindow::savesOrExtdata(), gameID);

    if ((lastUploadTime.toString("yyyy") != "1970" &&
         lastUploadTime.toString("yyyy") != "") &&
        (lastServerUploadTime.toString("yyyy") != "1970" &&
         lastServerUploadTime.toString("yyyy") != "") &&
        (lastDownloadTime.toString("yyyy") != "1970" &&
         lastDownloadTime.toString("yyyy") != "")) {
        if (lastServerUploadTime > lastDownloadTime) {
            QMessageBox msgBox;
            QString message =
                "You have uploaded to the server on a different device, but "
                "not downloaded it here.\nIt will overwrite whatever is on the "
    ...
    while (!directoriesToVisit.isEmpty()) {
        ...
        QStringList entryList = currentDir.entryList(QDir::Files | QDir::Dirs |
                                                     QDir::NoDotAndDotDot);
        ...
        for (const QString &entry : entryList) {
            ...
            if (QFileInfo(fullPath).isDir()) {
                ...
                QString relativePath =
                    QDir(directory).relativeFilePath(fullPath) + "/" +
    ...
    int responseCode = citraholdServer->uploadMultiple(
        MainWindow::savesOrExtdata(), allFilesArray);
    qDebug() << "Response code: " << responseCode;

    if (responseCode == 201) {
        MainWindow::showSuccessBox("Upload successful!");

        configManager->updateConfigProperty(
            "lastType",
            (savesOrExtdata() == UploadType::SAVES ? "SAVES" : "EXTDATA"));
        configManager->updateConfigProperty(
            "lastUploadedGameID", ui->uploadGameIDComboBox->currentText());
        configManager->updateConfigProperty("lastMode", "UPLOAD");

        QDateTime currentDateTime = citraholdServer->getLastUploadTime(
            MainWindow::savesOrExtdata(),
            ui->uploadGameIDComboBox->currentText());
        configManager->setLastUploadTime(
            MainWindow::savesOrExtdata(),
            ui->uploadGameIDComboBox->currentText(), currentDateTime);
        configManager->setLastDownloadTime(
            MainWindow::savesOrExtdata(),
            ui->uploadGameIDComboBox->currentText(),
            currentDateTime.addSecs(1));
        changeLastUploadText();

        handleServerFetch();
    } else {
        showErrorBox("Something went wrong during upload, HTTP " +
...
void MainWindow::handleServerFetch() {
    ...
    QVector<QString> *serverGameIDs =
        (MainWindow::savesOrExtdata() == UploadType::SAVES)
            ? &citraholdServer->serverGameIDSaves
            : &citraholdServer->serverGameIDExtdata;
    ...
    if (ui->uploadGameIDComboBox->currentText() ==
        ui->downloadGameIDComboBox->currentText()) {
...
void MainWindow::handleDownloadGameIDMissing() {
    showErrorBox("You need to add " +
                 ui->downloadGameIDComboBox->currentText().trimmed() +
                 " to your gameID list.");
...
void MainWindow::handleDownloadButton() {
    ...
    QJsonArray gameIDsOnFile =
        configManager->getGameIDFile(MainWindow::savesOrExtdata())["gameID"]
            .toArray();
    ...
    if (!gameIDpresent) {
        ...
        if ((lastUploadTime.toString("yyyy") != "1970" &&
            ...
            if (lastDownloadTime > lastServerUploadTime) {
                ...
                QString message = "You've already downloaded this save.\nAre "
                                  "you sure you want to download it again? It "
        ...
        configManager->copyDirectory(
            configManager->getGamePathFromGameID(
                MainWindow::savesOrExtdata(),
                ui->downloadGameIDComboBox->currentText()),
            (configManager->getOldSaveDirectory(MainWindow::savesOrExtdata()) /
             std::filesystem::path((ui->downloadGameIDComboBox->currentText() +
                                    "-" + formattedDateTime)
                                       .toStdString())));

        std::filesystem::remove_all(configManager->getGamePathFromGameID(
            MainWindow::savesOrExtdata(),
            ui->downloadGameIDComboBox->currentText()));
        if (!std::filesystem::exists(configManager->getGamePathFromGameID(
                MainWindow::savesOrExtdata(),
                ui->downloadGameIDComboBox->currentText()))) {
        ...
        if (downloadResponse) {
            ...
            std::filesystem::remove_all(configManager->getGamePathFromGameID(
                ...
                ui->downloadGameIDComboBox->currentText()));
            configManager->copyDirectory(
                (configManager->getOldSaveDirectory(
                     MainWindow::savesOrExtdata()) /
                 std::filesystem::path(
                     (ui->downloadGameIDComboBox->currentText() + "-" +
                      formattedDateTime)
...
void MainWindow::openAboutWindow() { aboutWindow->exec(); }

void MainWindow::openConfigFolder() {
    QDesktopServices::openUrl(QUrl::fromLocalFile(
        configManager->getSaveDirectory().string().c_str()));
}

void MainWindow::changeLastServerUploadText(bool upload) {

    QComboBox *comboBox =
        upload ? ui->uploadGameIDComboBox : ui->downloadGameIDComboBox;
    QLabel *label =
        upload ? ui->uploadLastUploadedLabel : ui->downloadLastUploadedLabel;
    ...
    QDateTime lastUploadTime = citraholdServer->getLastUploadTime(
        MainWindow::savesOrExtdata(), gameID);
    ...
    if (lastUploadTime.toString("yyyy") == "1970" ||
        ...
        label->setText("Last upload on server: " +
                       lastUploadTime.toString("dd/MM/yyyy hh:mm:ss"));
